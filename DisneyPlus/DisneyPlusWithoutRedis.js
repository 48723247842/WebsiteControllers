...
class DisneyPlus {
	...
	save_current_time( current_time="0" , time_remaining="0"  ) {
		return new Promise( async function( resolve , reject ) {
			try {
				if ( !this.now_playing ) {
					// We got here because the previous process crashed or something, and the video is still playing
				...
				await this.save_library_to_redis();
				this.time_update_stream.push( current_time );
				if ( this.time_update_stream.length > 30 ) {
					this.time_update_stream.shift();
				}
				resolve();
				return;
			}
	...
	pause() {
		return new Promise( async function( resolve , reject ) {
			try {
				...
				this.playing = false;
				setTimeout( () => {
					for ( let i = ( this.time_update_stream.length - 1 ); i > 0; --i ) {
						// Check to Make Sure it was paused
					}
					resolve();
					return;
				} , 3000 );
